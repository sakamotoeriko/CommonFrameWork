//
//  CommonConst.swift
//  CommonFrameWork
//
//  Created by sayi on 2018/05/20.
//  Copyright © 2018年 sayi. All rights reserved.
//

import Foundation
class CommonConst {
    
    static let DB_NAME: String = "app.db"
    
    //ベーシックURL
    static let APIBaseURL: String = "https://httpbin.org"
    
    //Login
    static let APILogin: String = APIBaseURL + ""
    
    //アルバム
    static let APIAlbum: String = APIBaseURL + ""
    
    //記事新聞
    static let APINews: String = "https://qiita.com/api/v2/users"
    
    //写真
    static let APIImage: String = "http://ipv4.download.thinkbroadband.com/10MB.zip"
    
    //アップロードURL
    static let APIUPLOAD_URL: String = "http://192.168.3.8/iosFileServer/uploadswift.php"
}
